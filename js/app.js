...
function sendResultToDrive(){
  const endpoint = window.RESULTS_ENDPOINT;
  const statusEl = $("syncStatus");
  if (!endpoint){
    if (statusEl) statusEl.textContent = "Not configured — showing local copy only.";
    return;
  }
  if (statusEl) statusEl.textContent = "Syncing to admin sheet…";
  fetch(endpoint, {
    method: "POST",
    mode: "no-cors", // Apps Script web apps don't return CORS headers; we don't need to read the response
    headers: { "Content-Type": "text/plain;charset=utf-8" }, // keeps this a "simple request" — no preflight
    body: JSON.stringify(state.lastResult)
  }).then(() => {
    // mode:'no-cors' gives an opaque response, so we can't confirm success —
    // treat the fetch not throwing as "sent".
    if (statusEl) statusEl.textContent = "Synced to admin sheet.";
  }).catch((err) => {
    console.warn("Could not sync result to Drive:", err);
    if (statusEl) statusEl.textContent = "Could not reach admin sheet — local copy still downloaded below.";
  });
}
